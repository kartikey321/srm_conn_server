...
import '../routes/thread/index.dart' as thread_indexre.dart' as middleware;
...
Handler buildRootHandler() {
  ...
  final router = Router()
    ..mount('/faculty/details', (context) => buildFacultyDetailsHandler()(context))
    ..mount('/faculty/email', (context) => buildFacultyEmailHandler()(context))
    ..mount('/faculty/updateclass', (context) => buildFacultyUpdateclassHandler()(context))
...
Handler buildFacultyDetailsHandler() {
  ...
  final router = Router()
    ..all('/', (context) => faculty_details_index.onRequest(context,))..all('/<id>', (context,id,) => faculty_details_$id.onRequest(context,id,));
  return pipeline.addHandler(router);
}
...
Handler buildFacultyEmailHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/<id>', (context,id,) => faculty_email_$id.onRequest(context,id,));
  return pipeline.addHandler(router);
}
...
Handler buildFacultyUpdateclassHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/', (context) => faculty_updateclass_index.onRequest(context,));
  return pipeline.addHandler(router);
}
...
Handler buildFacultyHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/', (context) => faculty_index.onRequest(context,));
  return pipeline.addHandler(router);
}
...
Handler buildMailHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/', (context) => mail_index.onRequest(context,))..all('/<id>', (context,id,) => mail_$id.onRequest(context,id,));
  return pipeline.addHandler(router);
}
...
Handler buildParentHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/', (context) => parent_index.onRequest(context,))..all('/<id>', (context,id,) => parent_$id.onRequest(context,id,));
  return pipeline.addHandler(router);
}
...
Handler buildStudentHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/', (context) => student_index.onRequest(context,))..all('/<id>', (context,id,) => student_$id.onRequest(context,id,));
  return pipeline.addHandler(router);
}
